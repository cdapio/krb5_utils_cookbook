  ...
  node['krb5_utils'][kt].each do |name, opts|
    ...
    when 'krb5_service_keytabs'
      if node['krb5_utils']['add_http_principal']
        http_principal = "HTTP/#{node['fqdn']}@#{node['krb5']['default_realm'].upcase}"
      else
        http_principal = ''
      end
      principal = "#{name}/#{node['fqdn']}@#{node['krb5']['default_realm'].upcase}"
      keytab_file = "#{name}.service.keytab"
      randkey = '-randkey'
    when 'krb5_user_keytabs'
      http_principal = ''
      principal = "#{name}@#{node['krb5']['default_realm'].upcase}"
